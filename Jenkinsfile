pipeline {
  ...
  stages {
    ...
    stage('Deploy to Kubernetes') {
      steps {
        // Jenkins user must be able to run kubectl (we verified/copié kubeconfig)
        sh "kubectl config use-context minikube || true"
        // apply all your k8s manifests (adjust names if necessary)
        ...
        // restart the spring deployment to pick new image tag
